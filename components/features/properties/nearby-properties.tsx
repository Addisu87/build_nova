...
export function NearbyProperties({ currentProperty, nearbyProperties }: NearbyPropertiesProps) {
  ...
  const PropertyFeature = ({ icon: Icon, label, value }: { icon: any; label: string; value: string | number }) => (
    <div className="flex items-center gap-2">
      <Icon className="h-4 w-4 text-gray-500" />
      <span className="text-sm text-gray-600">{label}: <span className="font-medium text-gray-900">{value}</span></span>
    </div>
  )
      ...
      <div className="overflow-y-auto flex-1">
        <Accordion type="single" collapsible className="divide-y">
          {nearbyProperties.map((property) => (
            <AccordionItem key={property.id} value={property.id} className="border-none">
              <div className="p-4">
