...
export default function PropertyDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 pt-4">
      ...
      />
      {/* Add this near your image thumbnails */}
      <div className="max-w-[1600px] mx-auto px-2 lg:px-4 mt-4">
        <ImageUploadPanel
          folderPath={`properties/${params.id}`}
          onUploadComplete={
            handleImageUploadComplete
          }
          maxFiles={
            10 - (property?.images?.length || 0)
          }
        />
      </div>
    </div>
...
function isNearby(
  ...
  const dLat =
    ((prop2.latitude - prop1.latitude) *
      Math.PI) /
    180
  const dLon =
    ((prop2.longitude - prop1.longitude) *
      Math.PI) /
    180
  const a =
